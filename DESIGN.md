...
- **Base distributions.** Today: `StandardNormal`, `DiagNormal`, `UniformBox`, `LatticeBase` (with factories for FCC, diamond, BCC, HCP, hexagonal ice — Stage B closed `2026-04-22`).
...
- **Feature helpers** in `embeddings.py`: `circular_embed` (per-coord Fourier features on a periodic box) and `positional_embed` (sinusoidal scalar embedding for context). Stateless; consumed by user / Stage D conditioners.
...
- **Built-in conditioner (`MLP`).** Requires `context` to be a single JAX `Array` of shape `(..., context_dim)` or `None`. The MLP concatenates `context` into its input tensor, so nested structures would require flattening at call time, which we do not do. `_compute_gate_value` has the same restriction (it branches on `context.ndim`). Users whose context is naturally a scalar, a flat feature vector, or pre-flattened-by-them fit here.
