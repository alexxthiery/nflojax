...
1. **Lean and hackable.** Plain dataclasses and pure functions; no framework magic, no plugin registries, no runtime dispatch. The unit of surveyability is the *concept*, not the file. A single-concept module (one class, one numerical kernel) should be surveyable in one read — approximately ≤ 500 LOC. Package facades and catalogue modules (`nflojax/transforms/` gathering bijection modules, `nets.py` gathering conditioners) are lists, not abstractions; they can grow as new entries land, but each *entry* in a catalogue is held to the same ≤ 500-LOC bar. An entry exceeding that bar is the real signal that the abstraction is wrong or the entry should split into its own file.
...
- **Reference conditioners** today: `MLP` (no equivariance), `ResNet` (building block for MLP), `DeepSets` (permutation-invariant), `Transformer` (permutation-equivariant attention), and `GNN` (permutation-equivariant message passing). Users can always bring their own conditioner that satisfies the contract; the shipped implementations are starting points, not the only option.
- **Feature helpers** in `embeddings.py`: `circular_embed` (per-coord Fourier features on a periodic box) and `positional_embed` (sinusoidal scalar embedding for context). `ProductDomain.conditioner_features` is the default MLP feature map for product-domain builders, not intrinsic domain semantics.
...
### 5.2 Context at the flow layer is a PyTree; built-ins require Array

Two-tier contract, split cleanly by role:

- **At the flow layer.** `context` is any JAX PyTree or `None`. `Flow`, `Bijection`, `CompositeTransform`, and every coupling pass `context` through to each block that accepts it; they do not inspect it. This is how temperature / density / lattice / box / species are threaded to conditioners without the flow layer caring.
- **Built-in conditioner (`MLP`).** Requires `context` to be a single JAX `Array` of shape `(..., context_dim)` or `None`. The MLP concatenates `context` into its input tensor, so nested structures would require flattening at call time, which we do not do. The internal gate helper has the same restriction (it branches on `context.ndim`). Users whose context is naturally a scalar, a flat feature vector, or pre-flattened-by-them fit here.
- **Custom conditioners.** Accept any PyTree the user chooses; the flow threading above guarantees the PyTree arrives unchanged. A GNN conditioner reading `{"positions": (N, d), "box": (d,), "species": (N,)}` is the canonical case. `validate_conditioner` checks `context_dim` and `apply` shape; for custom conditioners whose input is not a flat vector, validation is opt-out (see §5.4).

The split is deliberate: built-ins target the 80% case (flat contexts) with the simplest possible code; the PyTree guarantee stays at the flow layer where it is cheap (pass-through), so users who bring their own conditioner pay nothing.
...
A conditioner is a Flax module with `__call__(x, context=None) -> (..., out_dim)`. The output dimension matches `required_out_dim` of the coupling it serves. Zero-initialised output layer so the whole flow is identity at init. `validate_conditioner` in `nets.py` traces the module against abstract shapes to check the contract; coupling initializers patch the output layer when needed to enforce identity-at-init. For conditioners whose input is not a flat vector (GNN, Transformer), validation is opt-out.
...
- `transforms/` — focused bijection implementation modules plus the public `nflojax.transforms` facade. *Not for* conditioner code, base-distribution code, or anything that reads energy.
- `splines.py` — rational-quadratic spline math. *Not for* coupling / distribution / flow logic; this file is a numerical kernel.
- `distributions.py` — base distributions: `StandardNormal`, `DiagNormal`, `UniformBox`, `ProductBase`, and `LatticeBase`. *Not for* unnormalised target densities (those live in applications).
- `domains.py` — flat product-domain coordinate metadata (`ScalarDomain`, `ProductDomain`) and the default MLP feature map used by product-domain builders. *Not for* target, molecule, energy, or optimizer semantics.
...
- `embeddings.py` — stateless feature transforms: `circular_embed`, `positional_embed`. *Not for* learnable embeddings (those live in `nets.py` as modules).
- `builders/` — public builder facade plus focused modules for assembly, flat flows, product-domain flows, and particle flows. *Not for* primitive construction logic; each builder is a thin composition.
- `scalar_function.py` — LOFT numerical kernel, parallel role to `splines.py`.
- `geometry.py` — `Geometry` value object: per-axis box bounds + per-axis periodicity flags. Numpy-backed configuration (not a PyTree, not traced). Consumed by `CircularShift`, `Rescale`, `CoMProjection`, `UniformBox`, `LatticeBase`, and `utils/pbc`. *Not for* metrics, cell matrices for triclinic cells, curved manifolds — those need sibling types.
- `utils/pbc.py` — periodic-box geometry: `nearest_image`, `pairwise_distance(_sq)`. *Not for* forces, energies, or neighbour lists with cutoffs that are used by energies (those are application code).
- `utils/lattice.py` — lattice-position generators used by `LatticeBase`. *Not for* lattice-specific physics (Madelung constants, defect structures, etc.).
...
1. **No energy term anywhere in `nflojax/`.** Grep: no `epsilon`, `sigma`, `beta*`, `lambda_lj`, `kT`, `kcal`, `angstrom`, `hartree`. A generic conditioner message mask such as `GNN.cutoff` is allowed; energy cutoffs and force-neighbour lists stay in applications.
...
- **Conditioner** — the neural module inside a coupling that maps the frozen half to per-scalar bijection parameters. Pluggable; contract checked by `nets.validate_conditioner` and by coupling initializers that patch output layers for identity-at-init.
...
- *2026-04-21* — §2.1 "500 LOC" rule amended. It now applies per *concept*, not per file. Package facades and catalogue modules (`nflojax/transforms/`, `nets.py`) can grow as lists, but each entry is still bound to the 500-LOC bar. The amendment matches the current transform package split without loosening the bar on new code.
- *2026-04-21* — `Geometry` dataclass introduced (new `geometry.py`). Axis-aligned box + per-axis periodicity; numpy-backed, not a PyTree. Landed *before* Stage A to avoid retrofitting every geometry-consuming primitive (`Rescale`, `UniformBox`, `LatticeBase`, `utils/pbc` all consume it). `CircularShift` retrofitted to carry a single `geometry: Geometry` field; legacy-ergonomic `CircularShift.from_scalar_box(coord_dim, lower, upper)` classmethod preserved for tests / callers.
...
- *2026-04-21* — **Stage A closed.** Four tasks landed: `Rescale` (fixed per-axis affine from `Geometry` to a canonical target range, A1); `Permutation(event_axis=-2)` generalisation (A3, landed in Stage-0); context-type contract narrowed to Array-for-builtins / PyTree-for-custom (A4, §5.2 rewritten); `CoMProjection` (A2, Convention (1) zero log-det + static `ambient_correction` helper). What landed: the full rank-N bijection toolkit needed for Stage B / E. What remained as follow-up: `LinearTransform` audit and, later, the transform-package split that now lives at `nflojax/transforms/`. What was learnt: (1) cheap targeted/default tests matter more than asking agents to infer a bespoke file-to-tests map; (2) for bijections between spaces of different intrinsic embedding dimension (like `CoMProjection`), the right default is *zero* log-det plus an explicit caller-applied constant, because conflating volume-element bookkeeping with the primitive's log-det silently double-counts in composition patterns (augmented coupling); (3) documentation at six contact points (class docstring, REFERENCE, USAGE, EXTENDING, INTERNALS, AGENTS Gotchas) is the minimum viable redundancy for a convention that will bite users if they miss it.
- *2026-04-22* — **Stage B closed.** Four tasks landed: `UniformBox` (B1, per-axis uniform on `Geometry`), `utils/pbc.py` (B4, `nearest_image` + `pairwise_distance` consuming `Geometry`), `utils/lattice.py` (B2, pure functions for the 5 crystals), `LatticeBase` + 5 factories (B3, Gaussian-perturbed lattice with optional `permute` for indistinguishability). New `nflojax/utils/` subdir. What landed: with Stages A + B together, a user can assemble a particle-flow base distribution (liquid `UniformBox` or solid `LatticeBase`), wrap it with the bijection toolkit (`Rescale`, `CoMProjection`, `CircularShift`, `SplitCoupling`), and target a `(N, d)` event in ambient or reduced coordinates. What was deferred: spherical truncation on `LatticeBase` (no concrete trigger). What was learnt: (1) the Stage-0 `Geometry` retrofit paid off — every Stage-A and Stage-B primitive consumes the same value object with no friction, no per-call axis-bookkeeping bugs; (2) splitting work into many small commits *during a session* and one consolidated `feat: close Stage X` commit at the end is the right compromise between PR ergonomics and review friction; (3) the `permute=True` indistinguishability constant on `LatticeBase` has the same "caller-applied" character as `CoMProjection.ambient_correction` — both are unphysical bookkeeping that gradient training is invariant to; documenting them together as a *family* in REFERENCE.md "When the constant matters" decision boxes is more useful than scattering the same caveat under each primitive.
- *2026-04-22* — **Stage C closed.** Two stateless feature transforms in `nflojax/embeddings.py`: `circular_embed(x, geometry, n_freq)` (per-coord Fourier features on a periodic box) and `positional_embed(t, n_freq, base=10_000)` (sinusoidal scalar embedding). API decision: `circular_embed` takes `Geometry`, not raw `(lower, upper)` — continues the Stage-0 retrofit pattern; one path, no overload, agent doesn't need to learn two ways. Both raise `ValueError` on `n_freq=0` rather than emit a silent zero-width output (would break downstream `jnp.concatenate`). What was deferred: a `mask_non_periodic` knob on `circular_embed` (post-v1 if a real use case appears). What was learnt: stateless feature transforms make better library code than learnable feature modules — they have no init contract, no params plumbing, no dtype gotchas, and the user-supplied conditioner can compose them however it likes (USAGE.md "Conditioner features" shows the canonical pattern). Stage C completion clears the way for Stage D's reference conditioners (DeepSets, Transformer, GNN) to land without any sub-feature reimplementation.
- *2026-09-19* — **Particle nets read circular features on a torus (v0.2.0).** `DeepSets`, `Transformer` and `GNN` take `geometry` and `circular_n_freq`; `build_particle_flow` passes the flow-frame cube to every conditioner factory. Stays within scope: it reuses the stateless `circular_embed` (§8 charter: features in `embeddings.py`, modules in `nets.py`) and adds no builder option; the default follows the geometry (on for a fully periodic box). What was learnt: a conditioner on a torus must be a function on the torus, not on the cube; the seam cost a factor 2.3 in ESS downstream (PLAN.md §11, 2026-09-19).
