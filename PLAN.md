...
- [ ] **G1.** **LJ13 end-to-end** in `../bgmat-clean/` (bgmat-clean milestone **MS2**). 13-atom free-cluster Lennard-Jones at kT = 0.1 with DeepSets conditioner. Two variants trained and compared:
...
- [ ] **G2.** **mW water port** in `../bgmat-clean/` (bgmat-clean milestone **MS3**). Reproduces bgmat's monatomic-water tutorial using nflojax's `LatticeBase.hex_ice`, a GNN conditioner, and `build_particle_flow` (periodic path). Replaces the old parity-test idea.
...
- A user can build `bgmat-clean`'s flows (LJ13 free cluster at MS2, mW water at MS3) using nflojax primitives + application-side GNN conditioner + application-side energy / training / marginal-inference, without modifying nflojax.
