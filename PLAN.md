...
- **Pattern B promoted to a builder** — *landed 2026-09-22 as `build_augmented_flow` (v0.4.0)*. The remaining half of the promotion, a `marginalise_aux_half(...)` inference helper, is **not** built: the consumer needs it only to report a marginal ESS, since the canonical augmented target has a normalised auxiliary factor and its joint `log Z` is already the physical one. Build it when a consumer asks for the marginal density itself. The private `_CoMEmbed` shim stays as-is; augmented flows do not use it.
...
- *2026-09-21*: **`SplitCoupling(feature_map=...)`: a conditioner may read something other than the transformed variable (v0.3.0).** Found by bgmat-clean's R1b, its crystal-representation rewrite.
  ...
  - **Also fixed here, from `8537cb8`:** `TestOrthogonalTransform::test_invertibility` failed on `main` in float32. Not a defect: all 11 of that class's tests pass under x64, and the round-trip error tracks the matrix norm (1.9e-3 at dim=4, 3.0e-3 at dim=8, where `scale=3` gives `||A||_F` of 12 and 26). Two matrix exponentials of a large skew matrix cost about three decimal digits in float32, so the flat 1e-5 bar was unreachable there. The tolerance now follows the precision (1e-9 under x64, 5e-3 otherwise) with the measurement recorded in the test; a real inversion bug would be O(1) and still fails. Owner of that feature: worth a look in case the intent was for the transform to be accurate to 1e-5 in float32, which it is not at this scale.
- *2026-09-22*: **`build_augmented_flow`: Pattern B is a builder, and its documented translation-invariance claim was false (v0.4.0).** Requested by bgmat-clean's MS3, where the conditioner seeing half the particles is the suspected ceiling at N >= 64.
  - **Change.** `nflojax/builders/augmented.py`: the event is doubled to `(2N, d)` and every `SplitCoupling` splits on the physical/auxiliary boundary (`split_axis=-2`, `split_index=N`), so each coupling conditions on a full copy of all `N` particles, and both partitions are exactly `N`, which makes per-token conditioners work for odd `N` too. Same frame as `build_particle_flow` (`Rescale` into the spline cube, `CircularShift` per layer), the same conditioner-factory contract and periodic/`linear_tails` guard, plus the `feature_map` hook from 0.3.0.
  - **Documentation defect found and fixed.** `EXTENDING.md`'s Pattern B section claimed the augmented construction is translation invariant, so no centre-of-mass handling is needed. It is not, twice over: a zero-centred Gaussian base is not translation invariant, and a spline acting on absolute coordinates with parameters from the other half is not translation equivariant. Measured on a two-layer flow with perturbed parameters, `|f(x + c) - f(x) - c|` is the size of the output itself. A consumer following that paragraph would have dropped the flat-direction handling a periodic crystal needs. The corrected text names the two application-side routes (interleaved centre-of-mass shifts, or a tether with an analytic `log Z` correction), and `tests/test_augmented.py::TestTranslation` pins the non-equivariance so the claim cannot return.
  - **Tests.** `tests/test_augmented.py`, 16: the split is on the boundary in every coupling and both halves are `N` for either swap; odd `N` builds here and raises for `build_particle_flow` with the same per-token conditioner; identity on the couplings at init; round trip and log-det against autodiff under x64; gradients reach every coupling; `feature_map` passes through; the base must be on `(2N, d)`; the periodic guard; and the translation test above.
  - **Not built:** the auxiliary target, any marginal density, and translation handling. Those are the application's, and EXTENDING.md says so.
